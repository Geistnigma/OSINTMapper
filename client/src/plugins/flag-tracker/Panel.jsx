...
// Constante de MODULE : `tr` n'y existe pas, et une valeur figée à l'import ne
// suivrait pas un changement de langue. Le libellé se résout au rendu, via
// `tr('flag.statut.' + cle)`.
const STATUSES = [
  { id: 'found', cle: 'trouve', icon: '✅', color: '#10b981' },
  { id: 'progress', cle: 'enCours', icon: '🔄', color: '#f59e0b' },
  { id: 'stuck', cle: 'bloque', icon: '🧱', color: '#ef4444' },
  { id: 'skipped', cle: 'passe', icon: '⏭️', color: '#64748b' },
];
...
function getEntityFields(entity, tr) {
  const fields = [];
  if (entity.label) fields.push({ key: 'label', label: tr('flag.champ.label'), value: entity.label });
  if (entity.description) fields.push({ key: 'description', label: tr('flag.champ.description'), value: entity.description });
  if (entity.notes) fields.push({ key: 'notes', label: tr('flag.champ.notes'), value: entity.notes });
  const m = entity.metadata || {};
  if (m.lat && m.lng) fields.push({ key: 'gps', label: tr('flag.champ.gps'), value: `${m.lat},${m.lng}` });
  if (m.address) fields.push({ key: 'address', label: tr('flag.champ.adresse'), value: m.address });
  if (m.photo) fields.push({ key: 'photo', label: tr('flag.champ.photo'), value: m.photo });
  // Scan for any other string metadata
  for (const [k, v] of Object.entries(m)) {
    if (typeof v === 'string' && v && !['status','reliability','credibility','photo','address','time','date'].includes(k) && !k.startsWith('_')) {
...
function EntityTab({ entity, entities, selectedId, updateEntity, theme: t, settings, isViewer }) {
  ...
  const fields = useMemo(() => getEntityFields(entity, tr), [entity]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <button disabled={isViewer} onClick={() => upMeta({ isFlag: !isFlag, flag: !isFlag ? generatedFlag : null, flagSourceKey: !isFlag ? sourceKey : null })} style={{
        ...
        {isFlag ? tr('flag.estUnFlag') : tr('flag.marquer')}
      </button>

      {isFlag && <>
        {/* Source field selector */}
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4, textTransform: 'uppercase' }}>{tr('flag.champSource')}</div>
        ...
        {/* Preview */}
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4, textTransform: 'uppercase' }}>{tr('flag.apercu')}</div>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          }}>📋 {tr('flag.copier')}</button>
        </div>

        {/* Stored flag */}
        {meta.flag && (
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4, textTransform: 'uppercase' }}>{tr('flag.enregistre')}</div>
...
function FullscreenPanel({ entities, theme: t, settings, caseId }) {
  ...
  // Sentinelle stable : un libellé traduit comme valeur d'état casserait le
  // filtre au changement de langue.
  ...
  const filtered = useMemo(() => {
    let list = flags;
    if (filterCat !== TOUTES) list = list.filter(f => f.category === filterCat);
    if (filterStatus !== TOUTES) list = list.filter(f => f.status === filterStatus);
    return list;
  ...
  const exportWriteup = () => {
    const lines = [`# ${ctfName} - ${tr('flag.writeup')}\n`, `**${tr('flag.writeup.date')}** : ${new Date().toLocaleDateString(getLocale())}  \n`, `**${tr('flag.writeup.score')}** : ${stats.points}/${stats.totalPoints} pts - ${stats.found}/${stats.total} ${tr('flag.writeup.challenges')}  \n`, `**${tr('flag.writeup.temps')}** : ${formatTimer(timer)}\n`, `---\n`];
    const byCat = {};
    flags.forEach(f => { (byCat[f.category] = byCat[f.category] || []).push(f); });
    for (const [cat, list] of Object.entries(byCat)) {
      lines.push(`\n## ${cat}\n`);
      list.forEach(f => { const st = STATUSES.find(s => s.id === f.status); lines.push(`### ${st?.icon||''} ${f.challenge} - ${f.points} pts\n`, `**${tr('flag.writeup.statut')}** : ${(st ? tr('flag.statut.' + st.cle) : '')}\n`); if (f.flag) lines.push(`**Flag** : \`${f.flag}\`\n`); if (f.notes) lines.push(`\n${f.notes}\n`); lines.push(''); });
    }
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: t.bg, color: t.text }}>
      ...
      <div style={{ padding: '12px 20px', borderBottom: `1px solid ${t.border}`, background: t.surface, display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, color: t.textMuted }}>{tr('flag.resume', { trouves: stats.found, total: stats.total, points: stats.points })} · {tr('flag.pattern')}<code style={{ background: t.surfaceAlt, padding: '1px 6px', borderRadius: 4, fontSize: 10 }}>{pattern}{'{...}'}</code></div>
        </div>
        ...
        <button onClick={() => setView('add')} style={{ padding: '6px 14px', fontSize: 11, fontWeight: 700, borderRadius: 8, border: 'none', cursor: 'pointer', background: t.accent, color: '#fff' }}>{tr('flag.challenge')}</button>
        <button onClick={exportWriteup} style={{ padding: '6px 12px', fontSize: 11, fontWeight: 600, borderRadius: 8, border: `1px solid ${t.border}`, cursor: 'pointer', background: t.surfaceAlt, color: t.textSecondary }}>📝 {tr('flag.writeup')}</button>
      </div>
      ...
      {view === 'add' && (
        <div style={{ flex: 1, overflow: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 520 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ fontSize: 14, fontWeight: 700 }}>{tr('flag.ajouterChallenge')}</div>
            <button onClick={() => setView('list')} style={{ padding: '4px 12px', fontSize: 11, borderRadius: 6, border: `1px solid ${t.border}`, background: t.surfaceAlt, color: t.text, cursor: 'pointer' }}>{tr('flag.retour')}</button>
          </div>
          <div><div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4 }}>{tr('flag.form.nom')}</div><input value={form.challenge} onChange={e => setForm({ ...form, challenge: e.target.value })} placeholder={tr('flag.ph.nomChallenge')} style={inp} /></div>
          <div><div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4 }}>{tr('flag.flagManuel')}</div><input value={form.flag} onChange={e => setForm({ ...form, flag: e.target.value })} placeholder={`${pattern}{...}`} style={{ ...inp, fontFamily: 'monospace' }} /></div>
          <div><div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4 }}>{tr('flag.form.lierFlag')}</div>
            <select value={form.linkedEntityId} onChange={e => setForm({ ...form, linkedEntityId: e.target.value })} style={inp}>
              <option value="">{tr('flag.aucun')}</option>
              {flagEntities.map(e => <option key={e.id} value={e.id}>🚩 {e.label} → {e.metadata?.flag || tr('flag.pasGenere')}</option>)}
            </select>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <div style={{ flex: 1 }}><div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4 }}>{tr('flag.form.categorie')}</div><select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} style={inp}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</select></div>
            <div style={{ width: 80 }}><div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4 }}>{tr('flag.form.points')}</div><input type="number" value={form.points} onChange={e => setForm({ ...form, points: parseInt(e.target.value) || 0 })} style={inp} /></div>
          </div>
          <div><div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4 }}>{tr('flag.form.statut')}</div>
            <div style={{ display: 'flex', gap: 4 }}>{STATUSES.map(s => <button key={s.id} onClick={() => setForm({ ...form, status: s.id })} style={{ flex: 1, padding: 6, fontSize: 11, fontWeight: 600, borderRadius: 6, border: `1px solid ${form.status === s.id ? s.color : t.border}`, background: form.status === s.id ? s.color + '20' : t.surfaceAlt, color: form.status === s.id ? s.color : t.textSecondary, cursor: 'pointer' }}>{s.icon} {tr('flag.statut.' + s.cle)}</button>)}</div>
          </div>
          <div><div style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, marginBottom: 4 }}>{tr('flag.form.notes')}</div><textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} rows={3} placeholder={tr('flag.ph.commandes')} style={{ ...inp, resize: 'vertical' }} /></div>
          <button onClick={addFlag} style={{ padding: 10, fontSize: 13, fontWeight: 700, borderRadius: 8, border: 'none', background: t.accent, color: '#fff', cursor: 'pointer' }}>🚩 {tr('flag.ajouter')}</button>
        </div>
      )}
      ...
      {view === 'list' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '8px 20px', display: 'flex', gap: 6, borderBottom: `1px solid ${t.border}` }}>
            <select value={filterCat} onChange={e => setFilterCat(e.target.value)} style={{ padding: '4px 8px', fontSize: 11, background: t.surfaceAlt, color: t.text, border: `1px solid ${t.border}`, borderRadius: 6 }}><option value={TOUTES}>{tr('flag.tout')}</option>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</select>
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: '4px 8px', fontSize: 11, background: t.surfaceAlt, color: t.text, border: `1px solid ${t.border}`, borderRadius: 6 }}><option value={TOUTES}>{tr('flag.tout')}</option>{STATUSES.map(s => <option key={s.id} value={s.id}>{s.icon} {tr('flag.statut.' + s.cle)}</option>)}</select>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '8px 20px' }}>
            {filtered.length === 0 && <div style={{ textAlign: 'center', padding: 40, color: t.textMuted, fontSize: 12 }}>{tr('flag.aucunChallenge')}</div>}
            {filtered.map(f => {
              ...
              return (
                <div key={f.id} style={{ padding: '10px 14px', background: t.surface, border: `1px solid ${f.status === 'found' ? '#10b98140' : t.border}`, borderRadius: 10, marginBottom: 6 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(f.flag || linked?.metadata?.flag || ''); }} title={tr('flag.copier')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: t.textMuted }}>📋</button>
                  ...
                  {isEdit && (
                    <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${t.border}`, display: 'flex', flexDirection: 'column', gap: 8 }}>
                      ...
                      <select value={f.linkedEntityId || ''} onChange={e => updateFlag(f.id, { linkedEntityId: e.target.value })} style={inp}>
                        <option value="">{tr('flag.aucunFlagLie')}</option>
                      ...
                      <textarea value={f.notes || ''} onChange={e => updateFlag(f.id, { notes: e.target.value })} rows={3} placeholder={tr('flag.ph.notes')} style={{ ...inp, resize: 'vertical', fontSize: 11 }} />
                    </div>
                  )}
                </div>
              );
